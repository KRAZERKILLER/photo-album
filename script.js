...
function createMemory(photo) {
  ...
  if (photo && photo.src) {
    mem.innerHTML = `
      ...
      <div class="caption" data-id="${photo.id}" contenteditable="true">${photo.caption}</div>
    `;
  } else {
...
// Caption auto-save handler
document.addEventListener("blur", async e => {
  const captionEl = e.target;
  if (captionEl.classList.contains("caption")) {
    const id = captionEl.dataset.id;
    const newCaption = captionEl.innerText.trim();
...
document.addEventListener("keydown", e => {
  if (e.target.classList.contains("caption") && e.key === "Enter") {
    e.preventDefault();
    e.target.blur();
  }
});
